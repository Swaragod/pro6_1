...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            user_doc_number_to_del = input('Введите номер документа - ')
            user_doc_number_to_del, deleted = delete_doc(user_doc_number_to_del)
            if deleted:
                print('Документ с номером "{}" был успешно удален'.format(user_doc_number_to_del))
            else:
                print('Документ с номером "{}" не найден'.format(user_doc_number_to_del))
        elif user_command == 'm':
            move_doc_to_shelf()
